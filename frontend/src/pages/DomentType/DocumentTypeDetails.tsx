...
export default function DocumentTypeDetails({ visible, onHide, type }: any) {
  ...
  const tabHeaderTemplate = (context: any) => ({
    className: `flex items-center cursor-pointer select-none px-6 py-4 border-b-2 font-black text-[11px] uppercase tracking-widest transition-all duration-500 rounded-t-2xl
      ${
        context.active
          ? "border-dgcc6 text-dgcc3 bg-dgcc13/40 shadow-[0_-4px_12px_-5px_rgba(16,185,129,0.1)]"
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        <div className="relative bg-gradient-to-br from-dgcc5 via-dgcc2 to-slate-900 p-10 overflow-hidden">
          {/* Cercles de décoration subtils */}
          <div className="absolute -top-20 -right-20 w-64 h-64 bg-dgcc7/10 rounded-full blur-3xl" />
          ...
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 backdrop-blur-md rounded-full border border-white/20 text-dgcc10 text-[9px] font-black uppercase tracking-[0.3em] mb-4">
              <Info size={12} className="text-dgcc7" />
            ...
            <div className="flex flex-wrap gap-3">
              <div className="bg-white/10 backdrop-blur-md px-4 py-2 rounded-2xl flex items-center gap-3 border border-white/10 shadow-inner">
                <div className="p-1.5 bg-dgcc7/20 rounded-lg">
                  <Hash size={14} className="text-dgcc9" />
                </div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                ...
                {type.entites_un?.map((e: any) => (
                  ...
                  >
                    <Building2 size={12} className="text-dgcc9" />
                ...
                {type.entites_trois?.map((e: any) => (
                  <div
                    ...
                    className="bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-xl flex items-center gap-2 border border-white/10 shadow-inner"
                  >
                    <GitMerge size={12} className="text-dgcc9" />
                    <span className="text-white text-[11px] font-bold uppercase tracking-tight">
                      {e.libelle}
                    </span>
                  </div>
        ...
        <div className="p-8 -mt-6 bg-white rounded-t-[2.5rem] relative z-20 shadow-[0_-20px_40px_-15px_rgba(0,0,0,0.1)]">
          ...
          >
            <TabPanel
              header="Métadonnées"
              leftIcon={<ShieldCheck size={18} className="mr-3 text-dgcc6" />}
              pt={{
            ...
            <TabPanel
              ...
              leftIcon={<ListChecks size={18} className="mr-3 text-dgcc6" />}
              pt={{
                headerAction: ({ context }: any) => ({
                  className: `
                  flex items-center cursor-pointer select-none px-5 py-3 border-b-2 font-bold text-sm transition-all duration-300 rounded-t-xl
          ...
          <div className="mt-10 space-y-6">
            <div className="flex items-start gap-4 p-5 rounded-[1.5rem] bg-slate-50 border border-slate-100 hover:border-dgcc10 transition-colors duration-300 group">
              <div className="p-3 bg-white rounded-xl shadow-sm group-hover:text-dgcc5 transition-colors">
                <Settings2 size={20} />
              </div>
            ...
            <Button
              ...
              onClick={onHide}
              className="w-full py-5 bg-slate-900 hover:bg-dgcc1 text-white font-black text-sm uppercase tracking-[0.2em] rounded-[1.5rem] transition-all duration-300 shadow-xl shadow-slate-200 hover:shadow-dgcc12 border-none outline-none"
            />
          </div>
        </div>
      </div>
