// pages/SousDirection/SousDirectionDetails.tsx
import { useState, useEffect } from "react";
import { Dialog } from "primereact/dialog";
import { Button } from "primereact/button";
import { SousDirection, Direction, Fonction } from "../../../interfaces";
...
export default function SousDirectionDetails({
  visible,
  onHide,
  sousDirection,
  directions,
  ...
  const direction = directions?.find(
    (d: Direction) => d.id === sousDirection?.direction_id,
  );

  const fetchFonctions = async () => {
    if (visible && sousDirection?.id) {
      try {
        const data = await getFunctionsBySousDirection(sousDirection.id);
        setFonctions(data);
      } catch (err) {
