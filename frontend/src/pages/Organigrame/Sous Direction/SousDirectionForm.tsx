// pages/SousDirection/SousDirectionForm.tsx
import React, { useState, useEffect } from "react";
import { InputText } from "primereact/inputtext";
import { Dialog } from "primereact/dialog";
import { Button } from "primereact/button";
import { Dropdown } from "primereact/dropdown";
import { Save, Split, Hash, Info, MapPin } from "lucide-react";
import { SousDirection, Direction } from "../../../interfaces";

type Props = {
  visible: boolean;
  onHide: () => void;
  onSubmit: (data: Partial<SousDirection>) => Promise<void>;
  refresh: () => void;
  initial?: Partial<SousDirection>;
  directions: Direction[];
  title?: string;
};

export default function SousDirectionForm({
  visible,
  onHide,
  onSubmit,
  refresh,
      ...
      header={
        <div className="flex items-center gap-2 text-slate-800 font-bold">
          <div className="bg-orange-100 p-2 rounded-lg">
            <Split size={20} className="text-orange-600" />
          </div>
          <span>{title}</span>
        </div>
      }
      ...
      <div className="pt-4 space-y-5">
        <div>
          <label className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-2">
            <MapPin size={16} className="text-orange-500" /> Direction de
            rattachement
          </label>
          <Dropdown
            value={directionId}
        ...
        <div>
          ...
          <InputText
            ...
            placeholder="Ex: Sous-direction des Ressources Humaines"
          />
        </div>
