// pages/Section/SectionDetails.tsx
import { useState, useEffect } from "react";
import { Dialog } from "primereact/dialog";
import { Button } from "primereact/button";
import { Section, Division, Fonction } from "../../../interfaces";
...
export default function SectionDetails({
  ...
  const division = divisions?.find(
    (d: Division) => d.id === section?.division_id,
  );
        ...
        header={
          <div className="flex items-center gap-2 text-slate-800 font-bold">
            <div className="bg-dgcc12 p-2 rounded-lg">
              <TableOfContents size={18} className="text-dgcc5" />
            </div>
            <span>Détails de la section</span>
          </div>
        }
        visible={visible}
        style={{ width: "600px" }}
          ...
          <div className="border-b border-slate-100 pb-4">
            ...
            {division && (
              <div className="flex items-center gap-2 mt-2 text-sm text-slate-600">
                <Layers size={14} className="text-dgcc7" />
                <span>Rattachée à : {division.libelle}</span>
              </div>
            )}
          </div>
