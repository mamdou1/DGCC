...
export default function DirectionForm({
  ...
  return (
    <Dialog
      header={
        <div className="flex items-center gap-2 text-slate-800 font-bold">
          <div className="bg-dgcc13 p-2 rounded-lg">
            <Building2 size={20} className="text-dgcc" />
          </div>
          <span>{title}</span>
        </div>
      }
    ...
    >
      <div className="pt-4 space-y-5">
        <div>
          <label className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-2">
            <Hash size={16} className="text-dgcc4" /> Code
          </label>
          <InputText
            value={code}
            onChange={(e) => setCode(e.target.value.toUpperCase())}
            className="w-full px-4 py-3 bg-slate-50 border border-slate-400 rounded-xl outline-none focus:ring-4 focus:ring-dgcc/10"
            placeholder="Ex: DRH, DAF, DSI"
          />
        </div>
        ...
        <div>
          <label className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-2">
            <Info size={16} className="text-dgcc4" /> Libellé{" "}
            <span className="text-red-500">*</span>
          </label>
          <InputText
            value={libelle}
            onChange={(e) => setLibelle(e.target.value)}
            className="w-full px-4 py-3 bg-slate-50 border border-slate-400 rounded-xl focus:ring-4 focus:ring-dgcc/10 outline-none"
            placeholder="Ex: Direction des Ressources Humaines"
          />
        </div>
        ...
        <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
          ...
          <Button
            ...
            onClick={handleSubmit}
            disabled={!libelle || loading}
            className="bg-dgcc text-white font-bold px-8 py-3 rounded-xl shadow-lg hover:bg-dgcc1 transition-all border-none"
          />
        </div>
      </div>
    </Dialog>
