// pages/Division/DivisionDetails.tsx
import { useState, useEffect } from "react";
import { Dialog } from "primereact/dialog";
import { Button } from "primereact/button";
import { Division, SousDirection, Fonction } from "../../../interfaces";
...
export default function DivisionDetails({
  ...
  const sousDirection = sousDirections?.find(
    (sd: SousDirection) => sd.id === division?.sous_direction_id,
  );

  const fetchFonctions = async () => {
    if (visible && division?.id) {
      try {
        const data = await getFunctionsByDivision(division.id);
        setFonctions(data);
      } catch (err) {
        ...
        header={
          <div className="flex items-center gap-2 text-slate-800 font-bold">
            <div className="bg-dgcc13 p-2 rounded-lg">
              <Layers size={18} className="text-dgcc5" />
            </div>
          ...
          <div className="border-b border-slate-100 pb-4">
            ...
            {sousDirection && (
              <div className="flex items-center gap-2 mt-2 text-sm text-slate-600">
                ...
                <span>Rattachée à : {sousDirection.libelle}</span>
              </div>
            )}
          </div>
