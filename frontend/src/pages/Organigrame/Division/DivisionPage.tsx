// pages/Division/DivisionPage.tsx
...
export default function DivisionPage() {
  ...
  const toggleDivision = async (division: Division) => {
    if (expandedDivision === division.id) {
      ...
      await loadSections(division.id);
    }
  };
  ...
  // HANDLERS - Division
  // ============================================
  const onEdit = async (payload: Partial<Division>) => {
    ...
    try {
      ...
      toast.current?.show({
        severity: "success",
        summary: "Mis à jour",
        detail: "Division modifiée avec succès",
      });
      setEditing(null);
  ...
  const onCreate = async (payload: Partial<Division>) => {
    try {
      ...
      toast.current?.show({
        severity: "success",
        summary: "Succès",
        detail: "Division créée avec succès",
      });
      setFormVisible(false);
    } catch (err: any) {
      toast.current?.show({
  ...
  // ============================================
  const filtered = allDivisions.filter((div) => {
    const matchesSearch =
      (div.libelle || "").toLowerCase().includes(query.toLowerCase()) ||
      (div.code || "").toLowerCase().includes(query.toLowerCase());

    const matchesSousDirection = selectedSousDirection
      ? div.sous_direction_id === Number(selectedSousDirection)
      : true;

    return matchesSearch && matchesSousDirection;
  });
  ...
  const isLoading = isLoadingDivisions || isLoadingSousDirections;
  const error = errorDivisions || errorSousDirections;
  ...
  return (
    <Layout>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 font-medium">
              Gestion hiérarchique des divisions
            </p>
          </div>
        </div>
        <Button
      ...
      <div className="space-y-4">
        {paginated.length > 0 ? (
          paginated.map((division) => {
            ...
            const sousDirection = allSousDirections.find(
              (sd) => sd.id === division.sous_direction_id,
            );
            const sections = sectionsMap[division.id] || [];
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleDelete(String(division.id));
                      }}
      ...
      <DivisionForm
        ...
        refresh={() => refetchDivisions()}
        initial={editing || undefined}
        sousDirections={allSousDirections}
        title={editing ? "Modifier la division" : "Nouvelle division"}
      />
