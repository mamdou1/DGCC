...
// ChangePassword.tsx - Mis à jour avec les couleurs dgcc
import React, { useState, useRef, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { Button } from "primereact/button";
import { Toast } from "primereact/toast";
...
export default function ChangePassword() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (username !== user?.username) {
        ...
        await updateUser({ username }, user.id, undefined);
        hasChanges = true;
        toast.current?.show({
          severity: "success",
          summary: "Succès",
      ...
      if (hasChanges) await refreshUserData();
  ...
  // Helper pour remplacer les classes dynamiques de force de mot de passe
  const getStrengthBadgeClass = () => {
    if (passwordStrength.score <= 1) return "bg-red-100 text-red-700";
    if (passwordStrength.score === 2) return "bg-dgcc12 text-dgcc3";
    return "bg-dgcc13 text-dgcc";
  };
    ...
    <Layout>
      ...
      <div className="max-w-2xl mx-auto py-8 px-4">
        ...
        <div className="flex items-center gap-4 mb-8">
          <button
            onClick={() => navigate(-1)}
            className="p-3 bg-dgcc13 hover:bg-dgcc12 rounded-xl transition-all group"
            title="Retour"
          >
            <ArrowLeft
              size={20}
              className="text-dgcc group-hover:-translate-x-1 transition-transform"
            />
          </button>
          <div>
            <h1 className="text-3xl font-black text-dgcc flex items-center gap-3">
              <div className="p-3 bg-dgcc text-white rounded-2xl shadow-lg shadow-dgcc12/50">
                <Lock size={24} />
              </div>
              Modifier mon profil
            </h1>
            <p className="text-dgcc5 text-sm mt-1 ml-16">
              Mettez à jour vos informations de connexion
            </p>
          </div>
        </div>
        ...
        {/* Formulaire */}
        <div className="bg-white rounded-3xl shadow-xl border border-dgcc13 overflow-hidden">
          <div className="p-8 space-y-6">
            {errors.general && (
              <div className="p-4 bg-red-50 border border-red-200 rounded-xl flex items-center gap-3">
                <AlertCircle size={20} className="text-red-500 flex-shrink-0" />
            ...
            <div className="space-y-4">
              <h2 className="text-lg font-bold text-dgcc2 flex items-center gap-2 border-b border-dgcc13 pb-2">
                <UserRound size={20} className="text-dgcc4" />
                Informations du compte
              </h2>

              <div className="space-y-2">
                <label className="text-xs font-bold text-dgcc3 uppercase tracking-wider flex items-center gap-2">
                  <UserRound size={14} className="text-dgcc5" />
                  Nom d'utilisateur
                </label>
                <div className="relative">
                  <UserRound
                  ...
                  <input
                    ...
                    className={`w-full pl-12 pr-4 py-4 bg-slate-50 border-2 rounded-xl outline-none transition-all text-sm ${
                      username !== user?.username
                        ? "border-dgcc7 bg-dgcc13/30 focus:border-dgcc focus:ring-4 focus:ring-dgcc13"
                        : "border-slate-200 focus:border-dgcc focus:ring-4 focus:ring-dgcc13"
                    }`}
                  />
                </div>
              </div>
            ...
            <div className="space-y-4">
              <h2 className="text-lg font-bold text-dgcc2 flex items-center gap-2 border-b border-dgcc13 pb-2">
                <Lock size={20} className="text-dgcc4" />
                Changer le mot de passe
              </h2>

              <div className="space-y-2">
                <label className="text-xs font-bold text-dgcc3 uppercase tracking-wider flex items-center gap-2">
                  <Key size={14} className="text-dgcc5" />
                  Ancien mot de passe
                </label>
                <div className="relative">
                  <Lock
                  ...
                  <button
                    type="button"
                    onClick={() => setShowOldPassword(!showOldPassword)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-dgcc"
                  >
                    {showOldPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                  </button>
                </div>
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-dgcc3 uppercase tracking-wider flex items-center gap-2">
                  <Shield size={14} className="text-dgcc5" />
                  Nouveau mot de passe
                </label>
                <div className="relative">
                  <Lock
                ...
                {newPassword && (
                  <div className="mt-4 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-xs font-bold px-2 py-1 rounded-full ${getStrengthBadgeClass()}`}
                      >
                        {passwordStrength.message}
                      </span>
                    </div>
                    <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                      <div
                        className="h-full transition-all duration-300 bg-dgcc"
                        style={{
                          width: `${(passwordStrength.score + 1) * 20}%`,
                        }}
                      />
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-dgcc3 uppercase tracking-wider flex items-center gap-2">
                  <CheckCircle2 size={14} className="text-dgcc5" />
                  Confirmer le mot de passe
                </label>
                <div className="relative">
                  <Lock
                  ...
                  <input
                    ...
                    className={`w-full pl-12 pr-12 py-4 bg-slate-50 border-2 rounded-xl outline-none transition-all text-sm ${
                      passwordsMatch && confirmPassword
                        ? "border-dgcc5 bg-dgcc13/20"
                        : "border-slate-200 focus:border-dgcc"
                    }`}
                  />
                </div>
              </div>
            ...
            {/* Conseils de sécurité */}
            {newPassword && (
              <div className="mt-6 p-5 bg-dgcc13/30 rounded-xl border border-dgcc13">
                <p className="text-[10px] font-black text-dgcc2 uppercase tracking-wider mb-3">
                  Conseils de sécurité
                </p>
                <ul className="space-y-2 text-xs text-dgcc1">
                  <li className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${newPassword.length >= 8 ? "bg-dgcc4" : "bg-slate-300"}`}
                    />
                    Au moins 8 caractères
                  </li>
                  <li className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${/\d/.test(newPassword) ? "bg-dgcc4" : "bg-slate-300"}`}
                    />
                    Un chiffre
                  </li>
                </ul>
          ...
          {/* Boutons d'action */}
          <div className="flex justify-end gap-3 p-6 bg-slate-50/50 border-t border-dgcc13">
            <Button
              label="Annuler"
              onClick={() => navigate(-1)}
              className="p-button-text text-slate-600 font-bold hover:bg-slate-100 px-6 py-3 rounded-xl"
            ...
            <Button
              ...
              onClick={handleSubmit}
              loading={loading}
              className="bg-dgcc hover:bg-dgcc2 text-white border-none px-8 py-3 rounded-xl shadow-lg shadow-dgcc12/50 transition-all font-bold"
            />
          </div>
        </div>
      </div>
