...
//                     />
//                   )}
//                   {/* Dans votre Sidebar, remplacez la partie du CAS 2.2 par ceci
//                   :*/}
//                   {can("document", "access") && (
...
//                             else {
//                               // ✅ UN SIMPLE BOUTON, PAS DE SIDETREE
//                               return (
//                                 <SidebarLink
//                                   icon={FileStack}
//                                   text="Dossier / Type de document"
//                                   to="/document"
...
/**
 * Sidebar mis à jour avec la nouvelle charte graphique DGCC (#4e3625)
 */
...
export default function Sidebar({ children }: SidebarProps) {
  ...
  const isUserAdmin = (user: User | null): boolean => {
    ...
    if (!droitLibelle) return false;
    const libelle = droitLibelle.toString().toLowerCase();
    return libelle.includes("admin") || libelle.includes("administrateur");
  };
  ...
  const hasAccessToDocument = (typeDoc: TypeDocument): boolean => {
    ...
    user?.agent_access?.forEach((access) => {
      if (access.direction_id) userEntityIds.direction.add(access.direction_id);
      if (access.sous_direction_id)
        userEntityIds.sousDirection.add(access.sous_direction_id);
      if (access.division_id) userEntityIds.division.add(access.division_id);
      if (access.section_id) userEntityIds.section.add(access.section_id);
    });
  ...
  const getUserAccessibleEntityIds = (user: User | null) => {
    ...
    user.agent_access?.forEach((access) => {
      if (access.direction_id) ids.direction.add(access.direction_id);
      if (access.sous_direction_id)
        ids.sousDirection.add(access.sous_direction_id);
      if (access.division_id) ids.division.add(access.division_id);
      if (access.section_id) ids.section.add(access.section_id);
    });
    return ids;
  };
  ...
  return (
    <aside className="h-screen sticky top-0">
      <nav
        className={`h-full flex flex-col bg-dgcc border-r border-dgcc2 shadow-2xl transition-all duration-300 ${
          expended ? "w-76" : "w-20"
        }`}
      >
        {/* Header - Logo et Toggle */}
        <div className="flex items-center justify-between p-4 h-20 bg-dgcc">
          <div
            className={`overflow-hidden transition-all duration-300 ${
              expended ? "w-full" : "w-0"
            }`}
          ...
          <button
            onClick={() => setExpended((v) => !v)}
            className="p-2 rounded-lg bg-dgcc2 text-dgcc13 hover:bg-dgcc3 hover:text-white transition-all shadow-lg"
          >
            {expended ? <ChevronFirst size={20} /> : <ChevronLast size={20} />}
          </button>
        </div>
          ...
          <SidebarContext.Provider value={{ expended, treeOpen, toggleTree }}>
            <ul className="space-y-1">
              <SidebarLink
                icon={LayoutDashboard}
                text="Tableau de bord"
                to={can("statistique", "access") ? "/" : "/welcome"}
                active={
              ...
              <div
                className={`my-4 border-t border-dgcc3/30 mx-2 ${
                  !expended && "hidden"
                }`}
              />
        ...
        {/* Footer - Profil Utilisateur */}
        <div className="p-4 bg-dgcc2/40 backdrop-blur-sm border-t border-dgcc3/50">
          <div className="flex items-center gap-3">
            <div className="relative">
              <img
                src={
                ...
                alt="Profil"
                className="w-12 h-12 rounded-xl object-cover ring-2 ring-dgcc5/50 shadow-sm"
              />
            </div>

            <div
              className={`transition-all duration-300 overflow-hidden ${expended ? "w-40 opacity-100" : "w-0 opacity-0"}`}
            >
              <p className="text-sm font-bold text-white truncate uppercase tracking-wider">
                {user?.prenom} {user?.nom}
              </p>
              <p className="text-xs text-dgcc11 truncate">{user?.email}</p>
            </div>
            {expended && (
              <MoreVertical
                size={18}
                className="text-dgcc9 cursor-pointer hover:text-white"
              />
            )}
          </div>
        </div>
  ...
  return (
    <Link to={to} className="block group">
      <li
        ...
        ${
          active
            ? "bg-dgcc3 text-white shadow-lg shadow-black/20 scale-[1.02]"
            : "text-dgcc12 hover:bg-dgcc2/60 hover:text-white"
        }`}
      >
        <Icon
          size={22}
          className={`flex-shrink-0 ${
            active ? "text-white" : "text-dgcc8 group-hover:text-dgcc13"
          }`}
        />

        <span
          className={`whitespace-nowrap transition-all duration-300 font-medium flex-1 ${expended ? "opacity-100 w-auto" : "opacity-0 w-0"}`}
        >
          {text}
        </span>
        ...
        {!expended && (
          <div className="absolute left-full rounded-md px-3 py-1.5 ml-6 bg-dgcc2 text-white text-sm invisible opacity-0 -translate-x-3 transition-all group-hover:visible group-hover:opacity-100 group-hover:translate-x-0 z-50 shadow-xl border border-dgcc4 whitespace-nowrap">
            {text}
          </div>
        )}
      </li>
...
function SidebarTree({ label, icon: Icon, children, badge }: any) {
  ...
  return (
    <li className="list-none">
      <button
        onClick={() => toggleTree(label)}
        className={`flex items-center w-full px-3 py-3 rounded-xl transition-all duration-200 text-dgcc12 hover:bg-dgcc2/40
          ${!expended && "justify-center"}
        `}
      >
        <Icon size={22} className="flex-shrink-0 text-dgcc8" />
        {expended && (
          <>
            <span className="ml-3 font-medium flex-1 text-left">{label}</span>
            {badge && <span className="mr-2">{badge}</span>}
            <span
              className={`transition-transform duration-300 ${open ? "rotate-180 text-white" : "text-dgcc5"}`}
            >
              <ChevronDown size={16} />
            </span>
          </>
      ...
      {open && expended && (
        <ul className="ml-6 mt-1 space-y-1 border-l border-dgcc3/40 pl-2 animate-in fade-in slide-in-from-left-2 duration-300">
          {children}
        </ul>
      )}
    </li>
