...
class HistoriqueController {
  static async list(req, res) {
    ...
    try {
      logger.debug("🔍 Récupération de l'historique (sans pagination)", {
        userId: req.user?.id,
        query: req.query,
      });

      const { agent_id, action, resource, date_from, date_to } = req.query;
      ...
      // ⚡ Ici on enlève limit et offset
      const rows = await HistoriqueLog.findAll({
        where,
        include: [
          {
            model: Agent,
      ...
      logger.info("✅ Historique récupéré (sans pagination)", {
        count: rows.length,
        userId: req.user?.id,
        duration: Date.now() - startTime,
      });
      ...
      return res.json({
        data: rows,
        total: rows.length, // juste pour info
      });
    } catch (err) {
      logger.error("❌ Historique list error:", {
        error: err.message,
